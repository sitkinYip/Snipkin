...
- 🔊 **音频压缩**：对音频文件进行码率压缩，支持 7 档音质（320k ~ 32k），输出 MP3、AAC、M4A、OGG、OPUS 五种格式。
...
### 🔊 任务三：音频压缩 (Compress Audio)

1. 切换到 **音频压缩** Tab，点击 **选择文件** 载入音频文件（支持 MP3、AAC、FLAC、WAV、M4A、OGG、WMA、OPUS、AIFF、ALAC）。
2. 选择 **输出音质**：从无损品质 (320k) 到极低品质 (32k) 共 7 档可选，码率越高音质越好、文件越大。
3. 选择 **输出格式**：MP3、AAC、M4A、OGG 或 OPUS。
4. 确认输出路径（系统自动生成带时间戳的文件名），也可手动修改或点击 **保存路径** 选择位置。
5. 点击 **开始压缩**，即可完成音频压缩。
