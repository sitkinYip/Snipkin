"""
snipkin.handlers.compress_audio_handler - 音频压缩的事件处理层（Flet 版）

本模块提供音频压缩功能的所有事件处理函数，包括：
- on_input_file_picked:  输入文件选择完成后的回调处理
- on_output_file_picked: 输出路径选择完成后的回调处理
- handle_compress_run:   "开始压缩"按钮点击处理

设计说明：
  所有函数接收 AppState 实例和 UI 控件引用作为参数，
  通过 state 读写应用状态，通过控件引用更新 UI 显示。
  核心的参数校验、ffmpeg 命令构建、命令执行逻辑由 snipkin.core.compress_audio_core 提供。
"""
...
from snipkin.core.compress_audio_core import (
    build_compress_ffmpeg_command,
    execute_ffmpeg,
    generate_compressed_output_path,
    validate_compress_params,
)
...
def on_input_file_picked(
    ...
    if result and len(result) > 0:
        ...
        state.compress_audio_input_path = file_path
        input_path_field.value = file_path
        _log(state, f"已选择输入音频文件：{file_path}")

        # 调用 core 层生成默认输出路径
        output_format = state.compress_audio_output_format
        default_output = generate_compressed_output_path(file_path, output_format)
        state.compress_audio_output_path = default_output
...
def on_output_file_picked(
    ...
    """
    if result:
        state.compress_audio_output_path = result
        output_path_field.value = result
        _log(state, f"输出路径已设置：{result}")
        state.page.update()


def handle_compress_run(state: AppState) -> None:
    """
    处理"开始压缩"按钮点击事件。
    ...
    """
    # ---- 从 state 收集参数 ----
    input_path = state.compress_audio_input_path.strip()
    output_path = state.compress_audio_output_path.strip()

    # ---- 调用 core 层校验参数 ----
    params, error = validate_compress_params(
        input_path=input_path,
        output_path=output_path,
    )
    if error:
        _log(state, error)
        show_toast(state, error.replace("❌ 错误：", ""), "#FF3B30")
        return
    ...
    command = build_compress_ffmpeg_command(
        input_path=input_path,
        output_path=output_path,
        output_format=state.compress_audio_output_format,
        audio_quality=state.compress_audio_quality,
    )

    _log(state, f"▶ 执行命令：{' '.join(command)}")

    # 禁用按钮，防止重复点击
    run_button_wrapper = state.compress_audio_run_button
    ...
    # 显示 Loading 覆盖层
    show_loading(state, "正在压缩音频...")

    # ---- 在子线程中调用 core 层执行命令 ----
    thread = threading.Thread(
        target=_run_compress_ffmpeg_in_thread,
...
def _run_compress_ffmpeg_in_thread(
    state: AppState,
    command: list[str],
    output_path: str,
) -> None:
    ...
    """
    def on_success():
        _log(state, f"压缩成功！输出文件：{output_path}")
        show_toast(state, "✅ 压缩成功！文件已保存", "#34C759")

    def on_error(message):
        _log(state, message)
        show_toast(state, "❌ 压缩失败，请展开日志查看详情", "#FF3B30")
    ...
    execute_ffmpeg(
        ...
        on_error=on_error,
        on_complete=lambda: _restore_compress_run_button(state),
    )


def _restore_compress_run_button(state: AppState) -> None:
    """
    恢复压缩按钮到可点击状态。
    ...
    if run_button_wrapper is not None:
        ...
        inner_button.content = ft.Text("开始压缩", size=15, weight=ft.FontWeight.W_600)
